...
import { 
    ...
    Save, X, Contact, Shield, Archive, BookOpen
} from 'lucide-react';

export default function AdminSubjectForm({ navigate }: { navigate: (path: string) => void }) {
    ...
    const toggleGrade = (grade: string) => {
        setFormData(prev => {
            if (prev.applicableGrades.includes(grade)) {
                return { ...prev, applicableGrades: prev.applicableGrades.filter(g => g !== grade) };
            } else {
                return { ...prev, applicableGrades: [...prev.applicableGrades, grade] };
            }
        });
    };
                ...
                <header className="h-[72px] bg-white border-b border-gray-200 px-4 md:px-8 flex items-center justify-between sticky top-0 z-30">
                    <div className="flex items-center gap-4">
                        ...
                        <div className="flex items-center gap-3 hidden sm:flex">
                            <button onClick={() => navigate('admin_subjects')} className="text-gray-400 hover:text-[#1F3864] transition-colors">
                                <ChevronLeft className="w-5 h-5" />
                            </button>
                            <h2 className="text-xl font-bold text-[#1F3864]">Add / Edit Subject</h2>
                        </div>
                    </div>
                    <div className="flex items-center gap-4">
                    ...
                    <div className="bg-white rounded-xl border border-gray-200 shadow-sm overflow-hidden">
                        <div className="p-6 md:p-8 border-b border-gray-100 flex items-center gap-3 bg-[#1F3864] text-white">
                            ...
                            <div>
                                ...
                                <p className="text-sm text-blue-200 font-semibold mt-0.5">Enter academic subject configuration.</p>
                            </div>
                        </div>
                        ...
                        <form onSubmit={handleSubmit} className="p-6 md:p-8 space-y-8">
                            ...
                            <div className="space-y-6">
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                                    ...
                                    <div>
                                        <label className="block text-sm font-bold text-gray-700 mb-1.5">Subject Code</label>
                                        <input 
                                            type="text" 
                                    ...
                                    <div>
                                        <label className="block text-sm font-bold text-gray-700 mb-1.5">Subject Type</label>
                                ...
                                <div className="pt-2 border-t border-gray-100">
                                    <label className="block text-sm font-bold text-gray-700 mb-3">Applicable Grade Levels</label>
                                            ...
                                            <button
                                                ...
                                                className={`px-3 py-2 text-sm font-semibold rounded-lg border transition-colors ${
                                                    formData.applicableGrades.includes(grade)
                                                        ...
                                                        : 'bg-white border-gray-200 text-gray-600 hover:border-[#1F3864] hover:text-[#1F3864]'
                                                }`}
                                            >
                                                {grade}
                                            </button>
                                    ...
                                    {formData.applicableGrades.length === 0 && (
                                        <p className="text-red-500 text-sm mt-2 font-semibold">Please select at least one grade level.</p>
                                    )}
                                </div>
                            </div>
                            ...
                            <div className="pt-6 border-t border-gray-100 flex flex-col sm:flex-row gap-3 justify-between items-center">
                                <button 
                                    type="button"
                                    onClick={() => navigate('admin_subjects')}
